...
| Repudiation | An explanation hides a denied interval, restart, or replay conflict, or a callback is lost during native shutdown | First-class gaps, typed source identity, volume-bound stream mode, durable replay boundary, event IDs, policy binding, deterministic output, bounded callback queue, lifecycle records, named crash/replay matrix, owner-thread FSEvents shutdown fence | Selected-root lifecycle, blocked-summary, overflow-gap, volume-transition, boundary-mismatch, restart, and atomic rollback tests now; source-loss taxonomy and full recovery remain required |
