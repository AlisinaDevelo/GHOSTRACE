...
fn migration_is_idempotent_and_sqlite_safety_pragmas_are_set() {
    ...
    assert_eq!(journal.schema_version_count().expect("schema"), 5);
    assert_eq!(journal.schema_version().expect("user version"), 5);
    assert_eq!(journal.applied_migrations().expect("migration ledger").len(), 6);
    drop(journal);
    let reopened = Journal::open_fixture(&path, key).expect("reopen");
    assert_eq!(reopened.schema_version_count().expect("schema"), 5);
    assert_eq!(reopened.schema_version().expect("user version"), 5);
    assert_eq!(reopened.applied_migrations().expect("migration ledger").len(), 6);
