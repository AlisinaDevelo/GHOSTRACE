...
fn file_backed_journal_enforces_schema_pragmas_permissions_and_durable_state() {
    ...
    assert_eq!(journal.schema_version_count().expect("schema versions"), 5);
...
fn reopening_the_file_journal_reuses_the_same_migration_ledger() {
    ...
    assert_eq!(reopened.schema_version().expect("reopened version"), 5);
    assert_eq!(reopened.schema_version_count().expect("reopened schema"), 5);
}
